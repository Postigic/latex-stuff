...
\begin{proof}
    ...
    \end{description}
    By the inductive hypothesis, the player facing $4(k-1)+1$ loses; since $P_2$ has moved
    there, $P_1$ is the one facing $4(k-1)+1$, so $P_1$ loses and $P_2$ wins.
    ...
    \end{description}
    By the inductive hypothesis, the player facing $4k+1$ loses; since $P_1$ has moved there,
    $P_2$ is the one facing $4k+1$, so $P_2$ loses and $P_1$ wins.
